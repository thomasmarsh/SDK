                                                                   ...
                                                                   const vector<Touch::Ptr> &ignoreTouches)
    {
        ...
        for (const Touch::Ptr &recentTouch : recentTouchesSortedByEndTime) {
            if (touch->Id() == recentTouch->Id()) {
                ...
                MLOG_INFO(FTLogSDKClassificationLinker, "\t\t touch[%d] %s %s ->? touch[%d] %s %s reject 1 \n", (int)touch->Id(), ToString(touch->Phase()).c_str(), ToString(touch->CurrentClassification()()).c_str(), (int)recentTouch->Id(), ToString(recentTouch->Phase()).c_str(), ToString(recentTouch->CurrentClassification()()).c_str());
                break;
            }
            ...
            if (HasTouchProperty(recentTouch, kTouchDidExitOffscreen)) {
                ...
                MLOG_INFO(FTLogSDKClassificationLinker, "\t\t\t touch[%d] %s %s touchDidExitOffscreen decision: %d \n", (int)touch->Id(), ToString(touch->Phase()).c_str(), ToString(touch->CurrentClassification()()).c_str(), touchDidExitOffscreen);
            }
            ...
            if (enterToExitDistance > kMaxEnterToExitDistance) {
                ...
                MLOG_INFO(FTLogSDKClassificationLinker, "\t\t touch[%d] %s %s ->? touch[%d] %s %s reject 8 (%f > %f) \n", (int)touch->Id(), ToString(touch->Phase()).c_str(), ToString(touch->CurrentClassification()()).c_str(), (int)recentTouch->Id(), ToString(recentTouch->Phase()).c_str(), ToString(recentTouch->CurrentClassification()()).c_str(), enterToExitDistance, kMaxEnterToExitDistance);
        ...
        for (const Touch::Ptr &newTouch : recentTouchesSortedByStartTime) {
            ...
            if (HasTouchProperty(newTouch, kTouchDidEnterFromOffscreen)) {
                ...
                if (touchEntersFromOffscreenOptional) {
                    ...
                    MLOG_INFO(FTLogSDKClassificationLinker, "EnsurePenTouchContinuationLinkage newTouch[%d] %s %s, *touchEntersFromOffscreen: %d \n", (int)newTouch->Id(), ToString(newTouch->Phase()).c_str(), ToString(newTouch->CurrentClassification()()).c_str(), *touchEntersFromOffscreenOptional);
                }
            }
            ...
            if (touchEntersFromOffscreen &&
                ...
                MLOG_INFO(FTLogSDKClassificationLinker, "EnsurePenTouchContinuationLinkage newTouch[%d] %s %s -> oldTouch[%d] %s %s \n", (int)newTouch->Id(), ToString(newTouch->Phase()).c_str(), ToString(newTouch->CurrentClassification()()).c_str(), oldTouch ? (int)oldTouch->Id() : -1, oldTouch ? ToString(oldTouch->Phase()).c_str() : "None", oldTouch ? ToString(oldTouch->CurrentClassification()()).c_str() : "None");
            }
