...
TouchClassification TouchClassifierImpl::ClassifyPair(const Touch::cPtr & t0, const Touch::cPtr & t1, const TwoTouchPairType & type)
{
    ...
    return Convert(_Classifier->ClassifyPair(t0->Id(), t1->Id(), type));
}

TouchClassification TouchClassifierImpl::ClassifyForSingleTouchGestureType(const Touch::cPtr & touch, const SingleTouchGesture & type)
{
    return Convert(_Classifier->ClassifyForGesture(touch->Id(), type));
}
...
void TouchClassifierImpl::UpdateClassifications()
{
    ...
    for (const Touch::cPtr & touch : TouchTracker::Instance()->RecentTouches())
    {
        ...
        else
        {
            ...
            {
                *const_cast<Property<TouchClassification> *>(&touch->SingleTapClassification()) = Convert(_Classifier->ClassifyForGesture(touch->Id(), fiftythree::common::SingleTouchGesture::Tap));
                *const_cast<Property<TouchClassification> *>(&touch->LongPressClassification()) = Convert(_Classifier->ClassifyForGesture(touch->Id(), fiftythree::common::SingleTouchGesture::Longpress));
            }

            args.newValue = Convert(_Classifier->Classify(touch->Id()));
        }
...
common::TouchClassifier::Ptr common::TouchClassifier::New()
{
