...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverIncludedServicesForService:(CBService *)service
                                    error:(NSError *)error
{
    MLOG_INFO(FTLogSDK, "Peripheral did discover included services for service: %s", ObjcDescription(FTNameForServiceUUID(service.UUID)));
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service
                                   error:(NSError *)error
{
    MLOG_INFO(FTLogSDKVerbose, "Peripheral did discover characterisitics for service: %s", FTNameForServiceUUID(service.UUID).UTF8String);
...
- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic
                             error:(NSError *)error
{
    MLOG_INFO(FTLogSDKVerbose, "Peripheral did write value: %s for characteristic: %s", characteristic.value.description.UTF8String, FTNameForServiceUUID(characteristic.UUID).UTF8String);
                                          ...
                                          error:(NSError *)error
{
    if (characteristic.isNotifying) {
        MLOG_INFO(FTLogSDKVerbose, "Peripheral notification began on charateristic: %s", FTNameForServiceUUID(characteristic.UUID).UTF8String);
    } else {
        MLOG_INFO(FTLogSDKVerbose, "Peripheral notification stopped on characteristic: %s", FTNameForServiceUUID(characteristic.UUID).UTF8String);
    }
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverDescriptorsForCharacteristic:(CBCharacteristic *)characteristic
                                      error:(NSError *)error
{
    MLOG_INFO(FTLogSDK, "Peripheral did discover descriptors for characteristic: %s", ObjcDescription(FTNameForServiceUUID(characteristic.UUID)));
