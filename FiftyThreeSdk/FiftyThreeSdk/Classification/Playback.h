...
struct PlaybackEntry {
    ...
    PlaybackEntry(PlaybackEntryType type)
    : _type(type)
    {
    }

    PlaybackEntry(const std::set<core::Touch::Ptr> &touches)
    : _type(PlaybackEntryType::TouchesChanged)
    {
        for (const core::Touch::Ptr &touch : touches) {
            core::Touch::Ptr copyOfTouch = core::Touch::New(touch->Id(), touch->Phase(), touch->CurrentSample());

            std::string k;
            fiftythree::core::any v;
    ...
    PlaybackEntry(PenEvent const &penEvent)
    : _penEvent(penEvent)
    , _type(PlaybackEntryType::PenEvent)
    {
    }
