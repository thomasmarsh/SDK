...
std::pair<TouchClassification, float> PenEventClassifier::TypeAndScoreForTouch(core::TouchId touchId, PenEventIdSet &validPenEvents)
{
    ...
    std::set_intersection(penBeganSet.begin(), penBeganSet.end(), validPenEvents.begin(), validPenEvents.end(), std::inserter(validPenBeganSet, validPenBeganSet.begin()));
    ...
    if (_clusterTracker->TouchWithId(touchId)->IsPhaseEndedOrCancelled()) {
        ...
        std::set_intersection(penEndedSet.begin(), penEndedSet.end(), validPenEvents.begin(), validPenEvents.end(), std::inserter(validPenEndedSet, validPenEndedSet.begin()));
    ...
    float pDuration = DurationTimeErrorProbabilityForTouch(touchId, switchDuration, touchesBegan, priorBegan);
