...
void TouchClassificationProxy::SetClusterType(Cluster::Ptr const &cluster, TouchClassification newType, IdTypeMap &changedTypes)
{
    ...
    if (_activeStylusConnected) {
        ...
        if (cluster->_clusterTouchType == TouchClassification::Pen && newType != TouchClassification::Pen && TouchSize::IsPenGivenTouchRadius(*_clusterTracker->Data(cluster->_touchIds.back()))) {
...
void TouchClassificationProxy::ReclassifyCurrentEventGivenSize(IdTypeMap &changedTypes)
{
    ...
    for (Cluster::Ptr const &probeCluster : timeOrderedClusters) {
        ...
        if (!probeCluster->Stale()) {
            std::pair<TouchClassification, float> probePair = _penEventClassifier.TypeAndScoreForCluster(*probeCluster);
            auto probeTouch = probeCluster->MostRecentTouch();

            // todo: consistent use of most recent vs. cluster types...
            TouchData::Ptr data = _clusterTracker->Data(probeTouch);
            bool leadingSamplesArePenSized = TouchSize::IsPenGivenTouchRadius(*data);
            ...
            if (concurrentTouches.size() == 1) {
                possiblePan = ClassifyPair(concurrentTouches[0], probeTouch, TwoTouchPairType::AttemptedPan) == TouchClassification::Finger;

                possiblePinch = ClassifyPair(concurrentTouches[0], probeTouch, TwoTouchPairType::AttemptedPinch) == TouchClassification::Finger;
            }
    ...
    for (Cluster::Ptr const &probeCluster : timeOrderedClusters) {
        ...
        // Resolve "two pens at once" conflicts
        if ((probeClass == TouchClassification::Pen || probeClass == TouchClassification::Eraser) &&
            probeCluster->ContainsReclassifiableTouch()) {
            ...
            for (TouchId otherTouch : concurrentTouches) {
                ...
                if (newTypes.count(otherCluster) && otherTouch == otherCluster->MostRecentTouch()) {
                    otherClass = newTypes[otherCluster];
                }

                TouchData::Ptr const &otherData = _clusterTracker->Data(otherTouch);

                // we know probe is either a pen or eraser at this point
                if (probeClass == otherClass) {
                    // TODO: these radii tests aren't quite right for the eraser held straight up.
                    // however, in that case it seems quite difficult/unlikely to have palm touches.
...
void TouchClassificationProxy::DebugPrintClusterStatus()
{
    ...
    for (Cluster::Ptr const &cluster : timeOrderedClusters) {
        ...
        std::cerr << "([" << cluster->_id << ", " << strPhase << strType << "(" << cluster->_touchIds.size() << ")]: " << pair.second << ", " << cluster->_penPrior << ", dom = " << domScore << ") ";
    }
    std::cerr << "\n";
}
